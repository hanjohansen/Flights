...
{
    public async Task<TournamentState> CreateTournament(Guid tenantId, List<Guid> players, int firstRoundPlayersPerGame, GameType type, bool semiFinalWithLosersCup, int x01Target, InOutModifier inMod, InOutModifier outMod)
    {
        ...
        var model = TournamentModel.Create(tenantId, selectedPlayers, firstRoundPlayersPerGame, type, semiFinalWithLosersCup, x01Target, inMod, outMod);
    ...
    public async Task<TournamentState> ReshuffleLastRound(Guid tournamentId)
    {
        await using var db = await dbFactory.CreateDbContextAsync();
